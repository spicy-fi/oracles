...
export function getAssetPairIdByAssetIds(baseAssetId: string, quoteAssetId: string): AssetPair {
  const assetPair = assetPairs.find(
    (assetPair) => assetPair.baseAssetId === baseAssetId && assetPair.quoteAssetId === quoteAssetId,
  )

  if (assetPair === undefined) {
    throw new Error(`AssetPair with baseAssetId ${baseAssetId} and quoteAssetId ${quoteAssetId} not found`)
  }
...
export function getAssetPairSymbolById(id: number): string {
  ...
  return `${getAssetSymbolById(assetPair.baseAssetId)}${getAssetSymbolById(assetPair.quoteAssetId)}`
}
