...
describe("CoinGeckoProvider", () => {
  ...
  it("should handle API errors", async () => {
    ...
    await expect(coinGeckoProvider.fetchPrices(pairs)).rejects.toThrow("Request failed with status code 500")
  })

  it("should handle unexpected response data", async () => {
    setupNock(200, { bitcoin: { last_updated_at: 1684136889 } }) // no 'usd' field
    await expect(coinGeckoProvider.fetchPrices(pairs)).rejects.toThrow(UnexpectedResponseError)
  })
