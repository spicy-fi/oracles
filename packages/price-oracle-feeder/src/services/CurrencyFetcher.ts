...
import type { BulkCurrencyProvider, SinglePairCurrencyProvider } from "../providers/index.js"
...
class CurrencyFetcher {
  ...
  public async fetchMedianPrice(assetPair: AssetPair): Promise<AssetPairPrice> {
    ...
    const singlePairFetchPromises = this.singlePairProviders.map(async (provider) => {
      try {
        ...
        if (!fetchedAssetPairPrice) {
          console.log(
            `No valid price data for asset pair ${
              assetPair.id
            } ${getAssetPairSymbolById(assetPair.id)} from provider: ${provider.constructor.name}`,
          )
          return
        }

        if (Math.abs(Date.now() - fetchedAssetPairPrice.timestamp) > 60 * 60 * 1000) {
          console.log(
        ...
        console.error(`Error fetching price from provider: ${provider.constructor.name}`, err)
      }
    })
    ...
    const bulkFetchPromises = this.bulkProviders.map(async (provider) => {
      try {
        ...
        const fetchedAssetPairPrice = fetchedAssetPairPrices.find((f) => f.id === assetPair.id)
        ...
        if (Math.abs(Date.now() - fetchedAssetPairPrice.timestamp) > ONE_HOUR_IN_MILLISECONDS) {
          console.log(
            `Outdated price data for asset pair ${
              assetPair.id
            } ${getAssetPairSymbolById(assetPair.id)} from provider: ${provider.constructor.name}`,
        ...
        console.error(`Error fetching prices from provider: ${provider.constructor.name}`, err)
      }
    })
    ...
    if (assetPairPrices.length === 0) {
      throw new NoApiResponseError(`No valid price data for asset pair: ${assetPair.id}`)
    }
